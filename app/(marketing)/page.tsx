...
// FEATURE: Landing page — CTAs are now session-aware (route to /chatbots
// instead of /register if already logged in).
...
const features = [
  { title: "Any document, instantly indexed", desc: "Upload a PDF or text file. We parse, chunk, and embed it in seconds — no manual tagging." },
  { title: "Your own API key", desc: "Every chatbot ships with a scoped key. Track usage, set limits, revoke access — per bot." },
  { title: "Drop-in widget", desc: "One script tag. Matches your brand colors. No iframe hacks, no React required on your end." },
  { title: "Answers grounded in your docs", desc: "Retrieval + reranking means it cites your content — and says 'I don't know' instead of guessing." },
];

export default async function LandingPage() {
  const session = await getServerSession(authOptions);
  const ctaHref = session ? "/chatbots" : "/register";
  const ctaLabel = session ? "Go to your chatbots" : "Start free — no card required";
  ...
  return (
    <>
      <section className="py-24 md:py-32">
        <Container className="flex flex-col items-center text-center">
          ...
          <h1 className="font-display max-w-3xl text-4xl font-semibold leading-tight tracking-tight md:text-6xl">
            Turn your docs into a chatbot that knows them <span className="text-accent">cold</span>.
          </h1>
          ...
          <div className="mt-20 flex w-full max-w-2xl items-center justify-center gap-3 rounded-lg border border-line bg-surface px-6 py-5 font-mono text-xs md:text-sm">
            {pipeline.map((step, i) => (
              <div key={step} className="flex items-center gap-3">
                <span className={i === pipeline.length - 1 ? "text-accent" : "text-muted"}>{step}</span>
                {i < pipeline.length - 1 && <span className="pipeline-arrow text-accent-2">&rarr;</span>}
              </div>
            ))}
          </div>
        </Container>
      ...
      <section id="how-it-works" className="border-t border-line py-24">
        <Container>
          <h2 className="font-display text-center text-3xl font-semibold">What you actually get</h2>
      ...
      <section className="border-t border-line py-24">
        <Container className="flex flex-col items-center text-center">
          ...
          <p className="mt-4 max-w-md text-muted">Free plan includes 100 messages a month — enough to test it on a real file today.</p>
          <Button href={ctaHref} variant="primary" className="mt-8">
            {session ? "Go to your chatbots" : "Create your first chatbot"}
          </Button>
        </Container>
      </section>
    </>
